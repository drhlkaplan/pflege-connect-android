...
import {
  Stethoscope, User, MapPin, Search, MessageCircle, BookmarkCheck,
  Briefcase, Award, ChevronRight, Send, ArrowLeft, Building2, Eye, Bell
} from "lucide-react";
...
export default function NursePanel() {
  ...
  useEffect(() => {
    ...
    if (!loading && profile?.role !== "nurse") navigate("/dashboard");
  ...
  useEffect(() => {
    ...
    // Fetch CareScore
    supabase.from("nurse_profiles").select("care_score").eq("profile_id", profile.id).maybeSingle()
      .then(({ data }) => { if (data) setCareScore(data.care_score); });
    ...
    supabase.from("watchlist").select("id, watched_user_id, created_at").eq("user_id", user.id)
      .then(async ({ data }) => {
        ...
        const items = data.map(w => ({
          ...w,
          profile: profiles?.find(p => p.user_id === w.watched_user_id) || undefined
        }));
        setWatchlist(items);
      });
    ...
    supabase.from("nurse_profiles").select("id").eq("profile_id", profile.id).maybeSingle()
      .then(async ({ data: np }) => {
        if (!np) return;
        const { count } = await supabase.from("job_applications").select("id", { count: "exact", head: true }).eq("nurse_profile_id", np.id);
  ...
  const navigateToProfile = (userId: string, role: string) => {
    // Need to find the role-specific profile id
    if (role === "company") {
      supabase.from("company_profiles").select("id").eq("profile_id", userId).maybeSingle()
        .then(({ data }) => { /* fallback */ });
    }
    // For simplicity, navigate to messages
      ...
      <main className="container mx-auto px-4 pt-24 pb-12">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-display text-2xl font-bold">Hemşire Paneli</h1>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => {
            supabase.from("nurse_profiles").select("id").eq("profile_id", profile.id).maybeSingle()
              .then(({ data: np }) => {
                if (np) navigate(`/nurse/${np.id}#carescore`);
                else navigate("/nurse/profile");
              });
          }}>
            <CardContent className="p-4 text-center">
              <Award className="w-8 h-8 mx-auto mb-2 text-primary" />
              <p className="text-2xl font-bold">{careScore ?? "—"}</p>
          ...
          <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => navigate("/my-applications")}>
            <CardContent className="p-4 text-center">
              ...
              <p className="text-xs text-muted-foreground">Başvurularım</p>
            </CardContent>
          </Card>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-1">
            ...
            <CardContent className="space-y-2">
              <Button variant="outline" className="w-full justify-start" onClick={() => navigate("/companies")}>
                <Building2 className="w-4 h-4 mr-2" /> Şirket Ara
              </Button>
              <Button variant="outline" className="w-full justify-start" onClick={() => navigate("/jobs")}>
                <Briefcase className="w-4 h-4 mr-2" /> İş İlanları
              </Button>
              <Button variant="outline" className="w-full justify-start" onClick={() => navigate("/nurse/profile")}>
          ...
          <Card className="md:col-span-2">
            ...
            <CardContent>
              {watchlist.length === 0 ? (
                <p className="text-sm text-muted-foreground text-center py-4">İzleme listeniz boş. Profil sayfalarından kişileri izlemeye başlayın.</p>
              ) : (
