...
import {
  Building2, User, MapPin, Search, MessageCircle, BookmarkCheck,
  Briefcase, ChevronRight, Send, ArrowLeft, Stethoscope, Users, Plus, Bell
} from "lucide-react";
...
export default function CompanyPanel() {
  ...
  useEffect(() => {
    ...
    if (!loading && profile?.role !== "company") navigate("/dashboard");
  ...
  useEffect(() => {
    ...
    // Job postings count
    supabase.from("company_profiles").select("id").eq("profile_id", profile.id).maybeSingle()
      .then(async ({ data: cp }) => {
        if (!cp) return;
        const { count: jc } = await supabase.from("job_postings").select("id", { count: "exact", head: true }).eq("company_profile_id", cp.id).eq("is_active", true);
        setJobCount(jc || 0);
        const { count: ac } = await supabase.from("job_applications").select("id", { count: "exact", head: true })
          .in("job_posting_id", (await supabase.from("job_postings").select("id").eq("company_profile_id", cp.id)).data?.map(j => j.id) || []);
        setApplicationCount(ac || 0);
      });
  }, [profile, user]);
      ...
      <main className="container mx-auto px-4 pt-24 pb-12">
        <div className="flex items-center gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl pflege-gradient-company flex items-center justify-center">
              <Building2 className="w-5 h-5 text-primary-foreground" />
            </div>
            <h1 className="font-display text-2xl font-bold">Şirket Paneli</h1>
          </div>
        </div>

        {/* Quick Stats */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => navigate("/jobs/my")}>
          ...
          <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => navigate("/jobs/applications")}>
            <CardContent className="p-4 text-center">
              <Users className="w-8 h-8 mx-auto mb-2 text-primary" />
              <p className="text-2xl font-bold">{applicationCount}</p>
              <p className="text-xs text-muted-foreground">Başvurular</p>
            </CardContent>
          </Card>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <Card className="md:col-span-1">
            ...
            <CardContent className="space-y-2">
              ...
              <Button variant="outline" className="w-full justify-start" onClick={() => navigate("/jobs/create")}>
                <Plus className="w-4 h-4 mr-2" /> İlan Yayınla
              </Button>
              <Button variant="outline" className="w-full justify-start" onClick={() => navigate("/company/profile")}>
